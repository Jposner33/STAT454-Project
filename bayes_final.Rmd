...
```
```{r}
diag_post3 <- readRDS("~/Desktop/diag_post3.rds")
```
...
```{r}
combined_data_hs_clean_2 <- combined_data_hs %>%
  select(-c(patient_race))
```
...
```{r}
mcmc_acf(laplace_mod, pars = "DivisionNew England", "income_individual_median")
```
...
```
-We see here that 18 variables whose 80% credible intervals do not cross zero. We see 13 geographical predictors 11 states, and one division. One of these states Nevada is associated with a decrease chance of getting a diagonsis within 90 days, while Co,Ri,Dc,Ia, MO,Vt,Ne,De,Mn, and ID are all associated with a higher chance of getting a diagonsis within 90 days 
...
```
```{r}
tidy_mapdata2 <- tidyLaPlace_meaningful %>%
  mutate(state = substring(term, first = 6)) %>%
  filter(state == "CO" | state == "DC" | state =="RI" | state == "NV" | state == "MN"| state =="IA" | state == "MO" | state == "VT" | state =="DE")
```

```{r}
MapOfTidyStates <- plot_usmap(data = tidy_mapdata2, values = "odds", region = "state") + 
  labs(title = "States whose credible interval don't cross zero",
       subtitle = "Odds decrease/increase of diagonsis from red to pink") + 
  scale_fill_continuous(low = "red",high = "pink", name = "Odds change") +
  theme_minimal()

MapOfTidyStates
```

Looking at our table, and map we see that a few of our small states on the east coast (Dc,Ri,De) all have less than 30 cases. Vt the next state with the most cases has 53. The rest have at least 60 with MN and CO having over 300 cases. We see a small belt in the midwest, a collection of states near the rockies, and some east coast states but not super strong geographical trends. 

However, for patients living in MN as compared to AL is  8.975619 times more odds likely to get their diagonsis back within 90 days. 

Id has the strongest effect of any states, as a patient living in ID as comapred to AL is 4.648387e+32	more odds likely to get their diagonsis back within 90 days, and even though Idaho has almost 100 cases, this still is most likely an outlier. Looking at "America's Health ranking" 62.9% of Woman between 40-74 in Idaho received a screening. Ranking it 48th of all 50 us states, and this could mean that only patients who think they have breast cancer get screened, along with less overall patients leading to faster return time of diagonsis. 
...
```{r}
set.seed(43576)
classification_summary(laplace_mod, combined_data_hs_clean, cutoff = 0.56)
```
-   We notice an accuracy of 61% which is almost exactly the same as the simple Bayesian model we ran showing little difference between the two 
-Sensitivity 
...
-Overall we see certain states appear as meaninful vairbales, along with division NewEngland for geographical variables
-We see some patient specific variables such as Hispanic, age, and commercial insurance or not 
-We also see a few zipcode specific variables such as total hospitals, ozone, veterans, labor force, and density.
-We had 3 states that showed up in our final model Wisconsin, Illonsis, and Wyoming.
```{r}
state_case_count_model <- rename(combined_data_hs_clean, state = STATE) %>%
  group_by(state) %>%
  filter(DiagPeriodL90D != "n/a") %>%
  summarise(total = n()) 
  state_case_count_model %>%
    filter(state == "CO" | state == "DC" | state =="RI" | state == "NV" | state == "MN"| state =="IA" | state == "MO" | state == "VT" | state =="DE" | state == "ID")  %>%
    arrange(desc(total))
```
Both Wisconsin and Il had a negative correlation with a diagonsis within 90 days, while Wyoming had a positive correlation. However, Wyoming had only 1 cases, which is not enough for us to draw any trends 

Wisconsin had over 14 cases, and Il had 216 cases which makes us a little questionable about Wisconsin 

Wisconsin had an odds of 0.214, meaning that being from Wisconsin reduced your chance of getting a positive diagnosis within 90 days was 21.4% as high as someone from the average state, which is significanctly worse 

Il had an odds of 0.3195914 menaing that the average patient versus that same patient who lived in Il chance of getting a positive diagonsis within 90 days was 31.9% as high. 
...
```
Seeing as we have one case from New England this is also not nearly enough cases for us to explore the relation.

Moving onto patient specific data each increase in age by one year increased someone's chance of a positive diagnosis the odds are 1.005 higher. 

Having a commerical type of insurance 