...
combined_data_hs_clean <- na.omit(combined_data_hs_clean)
```
##Do not run the code below, instead make sure to download "laplace_modelreal.rds" from our Git Hub to your machine, as this model takes 3+ hours to run!
...
We see that the trace plots are very stable for one of the predictors that were kept in by the model (State Minnesota) and one that was kicked out (income_indivudal_median).
...
In general, this tidy plot is a lot to look at but looking through we can see a large amount of estimates that have been sent to very small values, and many predictors credible interval cross zero The intercept represents someone from AL, Asian, Midwest, on commercial insurance.
...
Patients living in MN as compared to AL are 8.975619 times more odds likely to get their diagnosis back within 90 days. 

ID has the strongest effect of any state, as a patient living in ID compared to AL is 4.648387e+32	more odds likely to get their diagnosis back within 90 days, yet only had 4 cases.
...
Seeing as we have only two cases from New England this is also not nearly enough cases for us to explore the relation. Moving onto patient-specific data each increase in age by one year increased someone's chance of a positive diagnosis the odds are 1.005 higher. Having MEDICARE ADVANTAGE type of insurance odds of getting a diagnosis within 90 days is 0.8569726 as high as someone with commercial insurance. Looking at zip code-specific variables for each increase in the year in age median of a zip code the odds likely of getting a diagnosis within 90 days was 0.8970481 as high. For each increase in the labor force by 1% (which is defined as the percent residents 16 or older in the workforce) the odds likely of getting a diagnosis with 90 was 0.9441394 as high. Looking at the percentage of residents who are veterans for each percent increase, the odds of getting a diagnosis within 90 days was 0.9682995 as high. For each 1 percentage increase in the number of residents who report their race as Native Hawaiian and Other Pacific Islander the odds likely of getting a diagnosis within 90 days was 227.46 percent higher.
